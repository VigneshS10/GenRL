...
class ObservationWrapper(gym.Env):
    ...
    def __init__(self, env, keys, relative=None, max_timesteps=10000):
        ...
        # self.env.sim.data.set_joint_qpos(f'object{1}:joint', [1.25, 0.53, 0.4, 1., 0., 0., 0.])
...
def test_policy(env, policy, n_rollouts, gamma=1, use_cum_reward=False, get_steps=False, max_timesteps=10000, stateful_policy=False, save_path="", rnd_no=0):
    '''
    ...
    succ_rate = 0.0
    num_steps = 0
    saved_points = [] #for plotting
    saved_actions = []

    for _ in range(n_rollouts):
        sarss = get_rollout(env, policy, False, max_timesteps=max_timesteps, stateful_policy=stateful_policy)
        num_steps += len(sarss)

        if stateful_policy:
            points = [list(state[:2]) for state in np.array([state for state, _, _, _ in sarss] + [sarss[-1][-1]])]
            actions = [list(action) for action in np.array([action for _, action, _, _ in sarss])]

            saved_points.extend(points)
            saved_points.append(["separator"])

            saved_actions.extend(actions)
            saved_actions.append(["separator"])
    ...
    # for x in saved_points:
    #         print(x)

    if stateful_policy:
        with open(f"{save_path}/test_rollouts_r{rnd_no}.pkl","wb") as fp:
            pickle.dump(saved_points,fp)
        
        with open(f"{save_path}/test_actions_r{rnd_no}.pkl","wb") as fp:
            pickle.dump(saved_actions,fp)

    return cum_reward / n_rollouts, succ_rate / n_rollouts


def print_performance(environment, policy, gamma=1, use_cum_reward=False, n_rollouts=100, max_timesteps=10000, stateful_policy=False, save_path = "", rnd_no = 0):
    avg_prob = 0.0
    for _ in range(10):
        environment.reset()
        reward, prob = test_policy(environment, policy, n_rollouts, gamma=gamma, use_cum_reward=use_cum_reward, max_timesteps=max_timesteps,
                                stateful_policy=stateful_policy, save_path=save_path, rnd_no=rnd_no)
        avg_prob += prob
    
    print('\n' + '-'*40)
    print('Estimated Reward: {}'.format(reward))
    # print('Estimated Reaching Probability: {}'.format(prob))
    print(f'Estimated average Reaching Probability round-{rnd_no}: {avg_prob/10}')
    print('-'*40 + '\n')
    return reward, prob
