...
class TaskSpec:
    ...
    def __init__(self, cons, predicate, subtasks, set_function = None):
        self.cons = cons
    ...
    def quantitative_semantics_dp(self, sys_traj, res_traj):
        ...
        if self.cons == Cons.seq:
            ...
            return retval
        if self.cons == Cons.seq2:
            subval1 = self.subtasks[0].quantitative_semantics_dp(sys_traj, res_traj)
            subval2 = self.subtasks[1].quantitative_semantics_dp(sys_traj, res_traj)
            subval3 = self.subtasks[2].quantitative_semantics_dp(sys_traj, res_traj)
            subval4 = self.subtasks[3].quantitative_semantics_dp(sys_traj, res_traj)
            subval5 = self.subtasks[4].quantitative_semantics_dp(sys_traj, res_traj)
            for i in range(n):
                for j in range(i, n):
                    retval[i][j] = -MAX_PRED_VAL
                    for k in range(i, j):
...
def for_loop(initial, phi, update, isnround):
    return TaskSpec(Cons.forloop, isnround, [TaskSpec(Cons.setting, None, [], initial), phi, TaskSpec(Cons.update, None, [], update)])