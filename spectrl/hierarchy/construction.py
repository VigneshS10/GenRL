...
def automaton_graph_from_spec(spec):
    ...
    '''
    if spec.cons == Cons.forloop:
        automaton, abstract_reach = automaton_graph_from_spec(spec.subtasks[1])
        abstract_reach.initial = spec.subtasks[0].set_function
        abstract_reach.update = spec.subtasks[2].set_function
        abstract_reach.isnround = spec.predicate

    elif spec.cons == Cons.ev:
        # Case 1: Objectives
